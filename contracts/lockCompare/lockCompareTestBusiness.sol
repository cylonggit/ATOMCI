...
contract EthBusiness {
    ...
    function incressAsset_undo_unlock(string _assetID,uint _num,string memory _invokeId,string memory _hashKey) public returns (bool){
     ...
     // ----------test contract lock  the follow will be lock the contract instead of resource  -------------------------------
    bool public contractLock;
    uint public countNum;
    function incressAsset_lock_do_contract(string _assetID,uint _num,string memory _invokeId,bytes32 _lockHash) public returns (bool){
        require(assetList[_assetID].valid && (!contractLock));
        //lock
        //string memory para=strConcat(para,_assetID);
        //para=strConcat(para,",");
        //para=strConcat(para,uintToString(assetList[_assetID].total));
    ...
    function incressAsset_unlock_contract(string _assetID,uint _num,string memory _invokeId,string memory _hashKey) public returns (bool){
        require(assetList[_assetID].valid && contractLock);
        require(keccak256(_hashKey)==assetList[_assetID].lockHash);
        // unlock
        contractLock=false;
    ...
    function incressAsset_undo_unlock_contract(string _assetID,uint _num,string memory _invokeId,string memory _hashKey) public returns (bool){
        require(assetList[_assetID].valid && contractLock);
        require(keccak256(_hashKey)==assetList[_assetID].lockHash);
        // undo
        string memory oldDataStr= assetList[_assetID].backup;
    ...
    function setContractLock(bool _lock) public returns (bool){
        contractLock=_lock;
        return true;
    }

    function incressCount() public returns (bool){
    ...
    //------------- read asset business unit--------------------
    event ReadAsset(string invokeId, string assetId,uint total);

    //atomic read
    function getAsset_atomic(string _assetID,string memory _invokeId) public returns(uint,bool,string memory){
        require(assetList[_assetID].valid );
        emit ReadAsset(_invokeId,_assetID,assetList[_assetID].total);
        return (assetList[_assetID].total, assetList[_assetID].ifLocked, assetList[_assetID].backup);
    }
    //original read
    function getAsset(string _assetID) public view returns(uint,bool,string memory){
        require(assetList[_assetID].valid );
        return (assetList[_assetID].total, assetList[_assetID].ifLocked, assetList[_assetID].backup);
    }
    ...
    function setcountNumber(uint _num) public returns (bool){
        countNum=_num;
        return true;
    }
