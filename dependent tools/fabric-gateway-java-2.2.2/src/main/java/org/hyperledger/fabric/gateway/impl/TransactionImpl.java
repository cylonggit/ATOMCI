...
public final class TransactionImpl implements Transaction {
    ...
    public void setCommitHandlerFactory(CommitHandlerFactory commitHandlerFactory) {
    ...
    public void setCommitTimeout(TimePeriod commitTimeout) {
    ...
    public TransactionImpl setEndorsingPeers2(final Collection<Peer> peers) {
        endorsingPeers = peers;
        return this;
    }
    ...
    public Collection<ProposalResponse> endorseTransaction(final String... args) {
        try {
    ...
    public Collection<ProposalResponse> sendTransactionProposal(final TransactionProposalRequest request)
    ...
    public byte[] commitTransaction(final Collection<ProposalResponse> validResponses)
    ...
    public OrderResponse commitTransactionV2(final Collection<ProposalResponse> validResponses)
            throws TimeoutException, ContractException, InterruptedException {
        OrderResponse re =new OrderResponse();
        BlockEvent.TransactionEvent event=null;
        ProposalResponse proposalResponse = validResponses.iterator().next();
        try {
            re.setResult(proposalResponse.getChaincodeActionResponsePayload());
        ...
        try {
            Channel.TransactionOptions transactionOptions = Channel.TransactionOptions.createTransactionOptions()
                    .nOfEvents(Channel.NOfEvents.createNoEvents()); // Disable default commit wait behaviour
            event=channel.sendTransaction(validResponses, transactionOptions)
                    .get(DEFAULT_ORDERER_TIMEOUT, DEFAULT_ORDERER_TIMEOUT_UNIT);
        ...
        commitHandler.waitForEvents(commitTimeout.getTime(), commitTimeout.getTimeUnit());
        re.setTransactionEvent(event);
        return re;

    }
    public TransactionProposalRequest newProposalRequest(final String... args) {
    ...
    public void configureRequest(final TransactionRequest request, final String... args) {
    ...
    public Collection<ProposalResponse> validatePeerResponses(final Collection<ProposalResponse> proposalResponses)
    ...
    @Override
    public ProposalResponse myEvaluate(final String... args) throws ContractException {
        QueryByChaincodeRequest request = newQueryRequest(args);
        Query query = new QueryImpl(network.getChannel(), request);
    ...
    public QueryByChaincodeRequest newQueryRequest(final String... args) {
